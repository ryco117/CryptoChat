...
int PeerToPeer::StartServer(const int MAX_CLIENTS)
{
	...
	while(ContinueLoop)
	{
		if(!GConnected && ConnectedClnt && ConnectedSrvr && SentStuff)	//All values have been sent, then set, but we haven't begun! Start already!
		{
			GConnected = true;
			cout << "\r          \r";
			currntLength = 0;
		...
		read_fds = master;
		if(select(fdmax+1, &read_fds, NULL, NULL, &zero) == -1)
		{
			cout << "\r";
		...
		for(unsigned int i = 0; i < MAX_CLIENTS + 1; i++)
		{
			if(MySocks[i] == -1)
				continue;
			if(FD_ISSET(MySocks[i], &read_fds))
			{
				if(i == 0)
				{
					...
					char TempValArray[1870] = {'\0'};
					string TempValString = "";
					while(TempValString == "")
					{
						recv(newSocket, TempValArray, 1870, 0);
						TempValString = TempValArray;
					}
					
					if(SentStuff) 	//We Sent Our Public Key And Get The Symmetric Key, Started Connection
					{
						SymKey = MyRSA.BigDecrypt(MyMod, MyD, mpz_class(TempValString));	//Decrypt The Symmetric Key With Our Public Key
					}
					else	//We Got The Public Key, We Came Late To The Party
					{
						ClientMod = mpz_class(TempValString.substr(0, 1250), 10);	//The leading zeros may confuse it with octal, specify decimal
						ClientE = mpz_class(TempValString.substr(1250, 620), 10);
					}
				}
				else
				{
					...
					if((nbytes = recv(MySocks[i], buf, 512, 0)) <= 0)		//handle data from a client
					{
		...
		if(!ConnectedClnt)		//Self expl
		{
			TryConnect();		//Lets try to change that
		}
		else if(!SentStuff && ClientMod != 0 && ClientE != 0)		//We have established a connection, we were late to party, and haven't sent stuff, but we have their keys!
		{
			string MyValues = (MyRSA.BigEncrypt(ClientMod, ClientE, SymKey)).get_str();	//Encrypt The Symmetric Key With Their Public Key
...
void PeerToPeer::DropLine(string pBuffer)
{
	...
	int i = pBuffer.length() - 1;
	for(; i >= 0; i--)
		if(pBuffer[i] != '\0')
			break;
	
	if((i+1) % 16 == 0)
		pBuffer.erase(i+1, (pBuffer.length() - i));	//Erase Any null terminators that we don't want to decrypt
	
	cout << "Client: " << MyAES.Decrypt(SymKey, pBuffer);
	cout << "\nMessage: " << OrigText;
...
void PeerToPeer::TryConnect()
{
	if(connect(Client, (struct sockaddr*)&socketInfo, sizeof(socketInfo)) >= 0) //connected
	{
		...
		string TempValues = "";
		string MyValues = "";
		if(!ConnectedSrvr)		//So We Are First
		{
			TempValues = MyMod.get_str();
			while(TempValues.length() < 1250)
				TempValues = "0" + TempValues;

			MyValues = TempValues;

			TempValues = MyE.get_str();
			while(TempValues.length() < 620)
				TempValues = "0" + TempValues;
			MyValues += TempValues;
